...
describe("Story 7.11 — debt compression", () => {
  it("AC2 — greedy matching produces at most n-1 transfers", () => {
    const transfers = compressDebts([
      { userId: "a", netMinor: fiatMinor(-100) },
      { userId: "b", netMinor: fiatMinor(-50) },
      { userId: "c", netMinor: fiatMinor(80) },
      { userId: "d", netMinor: fiatMinor(70) },
    ]);
  ...
  it("AC1 — operates on confirmed net positions only", () => {
    const transfers = compressDebts([
      { userId: "debtor", netMinor: fiatMinor(-1000) },
      { userId: "creditor", netMinor: fiatMinor(1000) },
    ]);
