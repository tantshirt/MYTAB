...
**U-10 · Wallet-standard first where injected; UL in any Telegram WebView without injection. · amended 2026-08-23; return unproven on a physical phone**
Solana Mobile's own docs state MWA is **unsupported on all iOS surfaces** (app and browser) because iOS suspends backgrounded apps and kills the local-socket session MWA needs. Safari Web Extensions can expose wallet-standard *inside Safari*; a Telegram Mini App is a WebView on **every OS**, so those extensions do not apply — including Android Telegram. **Device path chosen:** wallet-standard first when the named wallet is injected; otherwise Phantom / Solflare / Backpack **universal links** (the same three hosts). HTTPS `redirect_link` opens the mobile browser, not Telegram; the callback is stashed on the challenge row and the Mini App reopens via `startapp=ulcb_*`. Decrypt uses the dapp secret still in that WebView. Do not invent a fourth named wallet. Do not accept an address as an argument. **Return is still unproven until a physical phone confirms.** This is not a live-cluster wallet link.
